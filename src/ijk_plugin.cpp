...
//	useful defines
	...
	{
		// COG raster data user data
		string name = string("COGUserData").append(suffix);
		typedef COGUserData<number, dim> T;
		typedef typename T::base_type TBase;

		reg.template add_class_<T,TBase>(name, grp)
			.template add_constructor<void (*)()>("COGUserData")
			.template add_constructor<void (*)(const std::string&, int)>("COGUserData")
			.set_construct_as_smart_pointer(true);

		reg.add_class_to_group(name, "COGUserData", tag);
	}
